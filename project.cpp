...
class Machine
{
    ...
    Machine copy()
    {
        Machine clone(name,edges,vertices,final);
        return clone;
    }
    ...
    Machine FirstT()//премахване на епсилон преходите
    {
        ...
        //edges = newEdges;
        //final = f;
        Machine ans("",newEdges,vertices,f);
        return ans;
    }

    Machine SecondT()//детерминиране чрез Рабин-Скот
    {
        string n = "Determined->" + name;
        ...
        for(int i = 0;i<2*a[vertices-1];i++)
        {
            ...
            for(char c = 'a'; c <= 'z';c++)//за тестове е по-удобно азбуката да е {a,b}, иначе става огромно
            {
                int OtherVertex = 0;
    ...
    Machine transform(vector<Machine>& v) //мутатор
    {
        Machine m1 = FirstT();
        Machine m2 = m1.SecondT();
        v.push_back(m2);
        return m2;
    }
};
...
{
    if(t=="/") return Machine("Machine of </>", {}, 1, {});
    Machine res("Machine of <" + t + ">", {}, t.size()+1, {(int)t.size()});
...
Machine evaluate(string t) //Обратен полски запис
{
    stack<int> operators;
...
int main()
{
    Machine test1("test",{{0,1,'a'},{0,1,'b'}},2,{1});
    //test1.SecondT().printEdges();
    Machine test2("Test",{{0,1,'a'},{0,1,'b'},{0,2,'_'},{1,2,'a'},{1,2,'_'}}, 3, {2});
    //test2.FirstT().printEdges();
    File_Container F1;
    ...
    while(true)
    {
        ...
        else if(entry.size()>=5 && entry.substr(0,5)=="print")
        {
            ...
            //findMachine(entry,machines).printFinals();
        }
        ...
        else if(entry.size()>=2 && entry.substr(0,2)=="un")
        {
            entry.erase(0,3);
            cout << findMachine(entry,machines).un(machines) << endl;
        }

        else if(entry.size()>=9 && entry.substr(0,9)=="transform")
        {
            entry.erase(0,10);
            findMachine(entry,machines).transform(machines);
            cout << "You've successfully transformed " + entry + "!" << endl;
        }

        else if(entry.size()>=3 && entry.substr(0,3)=="reg")
        {
            entry.erase(0,4);
            Machine res = evaluate(entry);
            machines.push_back(res);
        ...
        else if(entry=="help")
        {
            ...
            cout << "list -> Gives you the list of the indicators of the automata." << endl;
            cout << "print <id> -> Presents an information for the transitions in an automaton." << endl;
            cout << "save <id> <filename> -> Saves an automaton in a file." << endl;
            cout << "exit -> Exits the program." << endl;
            cout << "empty <id> -> Determines if the language of an automaton is empty." << endl;
            cout << "finite <id> -> Determines if the language of an automaton is finite." << endl;
            cout << "deterministic <id> -> Determines if an automaton is deterministic." << endl;
            cout << "recognize <id> <word> -> Determines if a word belongs to the language of an automaton." << endl;
            cout << "union <id1> <id2> -> Prints an indicator of the union automaton of two automata." << endl;
            cout << "concat <id1> <id2> -> Prints an indicator of the concatenate automaton of two automata." << endl;
            cout << "un <id> -> Prints an indicator of the positive hull of an automaton." << endl;
            cout << "transform <id> -> Transforms an automaton to a Deterministic Finite Automaton." << endl;
            cout << "reg <expression> -> Generates an automaton from a regular expression." << endl;
            cout << "Open <filename> -> Opens a file." << endl;
            cout << "Write data -> Gives you the opportunity to write in the file." << endl;
            cout << "Save -> Saves the written data in file." << endl;
            cout << "SaveAs <filename> -> Saves the written data in another file." << endl;
            cout << "Close -> Closes the file." << endl;
        }

        else if(entry.size()>=4 && entry.substr(0, 4)=="Open")
        {
            entry.erase(0, 5);
            F1.Open(entry);
        }
        ...
        else if(entry.size()>=6 && entry.substr(0, 6)=="SaveAs")
        {
            entry.erase(0,7);
            F1.SaveAs(entry);
        }

        else if(entry=="Write data")
        {
            cout << "Write the data here, enter \% in the end:" << endl;
            F1.Write();
        }
